...
# 执行契约：模式 D (生成模式)
# 无 entry → LLM 按 SKILL.md body 直接生成 SVG → 写 diagram/<topic>/<topic>.svg
# 可选 post_process: scripts/main.ts 做 SVG→PNG 转换（需 bun）
params:
  - name: diagram_type
    type: string
    description: 图表类型，可选 architecture|flowchart|sequence|structural|mindmap|timeline|state_machine|data_flow|illustrative
    required: true
  - name: topic
    type: string
    description: 图表主题简述
...
output_ext: .svg
output_subdir: diagram
output_name: "{topic}"
post_process: scripts/main.ts
---
