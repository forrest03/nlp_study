...
# 执行契约：模式 A (script + data_file)
# LLM 按 data_instructions 生成单词 JSON → 写 data/<word>.json → 执行 make_flashcard.py
params:
  - name: word
    type: string
    description: 要制作闪卡的英语单词
    required: true

entry: scripts/make_flashcard.py
entry_input: data_file

data_instructions: |
  请为英语单词 "{word}" 生成学习闪卡数据，严格输出 JSON：
  {{
    "word": "{word}",
  ...
  {avoid_hint}
  要求：
  - 例句必须地道、长度适中、能体现该词典型用法；
  - 例句固定 3 条；
  - 近义词 4-6 个，尽量贴近该词在释义下的核心含义；
  - 所有中英文内容必须真实、准确，不要编造音标。
---
