...
VECTORSTORE_DIR = BASE_DIR / "vectorstore"/ "faiss_semantic"
        ...
        messages=[
            {
                "role": "system",
                "content": (
                        "你是法律检索查询优化专家。将用户的问题改写为更适合从法律文档（如法律法规、司法解释、裁判文书等）中检索信息的精确查询语句。"
                        "保留关键法律要素（法律名称、条款编号、罪名、主体、行为等），扩展相关法律术语和同义词，不要超过50字。"
                        "直接输出改写后的查询语句，不要解释。"
                            )
            },
...
class RAGPipeline:
    ...
    def query(
        ...
        if top_score < SCORE_THRESHOLD and filter_meta is None:
            return {
                "answer": "知识库未能找到与该问题相关的内容，建议直接查阅原始文档。",
...
def main():
    parser = argparse.ArgumentParser(description="法律文档 RAG 问答（原生版）")
    parser.add_argument("--query",         type=str,  default=None)
    parser.add_argument("--raw_name",      type=str,  default=None, help="中华人民共和国刑法")
    ...
    filter_meta = {}
    if args.raw_name: filter_meta["raw_name"] = args.raw_name
