...
class QueryRequest(BaseModel):
    question:   str           = Field(...,  json_schema_extra={"example":"正当防卫造成不法侵害人伤亡的，要坐牢吗？需要赔偿医药费吗？"})
    raw_name: Optional[str] = Field(None,json_schema_extra={"example":"中华人民共和国刑法"})
...
def _filter_meta(req: QueryRequest) -> Optional[dict]:
    fm = {}
    if req.raw_name: fm["raw_name"] = req.raw_name
    return fm or None
