...
BATCH_SIZE      = 8                  # DashScope text-embedding-v3 单次最多 10 条
DASHSCOPE_URL   = "https://ws-u1q6go2sf0t2vntq.cn-beijing.maas.aliyuncs.com/compatible-mode/v1"
...
def build_faiss_index(chunks: list[dict], client: OpenAI):
    ...
    index_path = VECTORSTORE_DIR / "faiss_semantic/faiss_index.bin"
    meta_path  = VECTORSTORE_DIR / "faiss_semantic/faiss_meta.json"
    ...
    meta_list = [
        {
            "chunk_id":   c["chunk_id"],
            "content":    c["content"],
            "raw_name": c["metadata"].get("raw_name", ""),
...
def main():
    ...
    logger.info("\n索引构建完成！")
    logger.info(f"  FAISS 索引: {VECTORSTORE_DIR / 'faiss_semantic/faiss_index.bin'}")
    logger.info(f"  元数据:     {VECTORSTORE_DIR / 'faiss_semantic/faiss_meta.json'}")
