...
def build_server_configs() -> dict[str, StdioServerParameters]:
    ...
    return {
        "region": StdioServerParameters(
            command=sys.executable,
            args=[str(servers / "region_server.py")],
            env={**os.environ},
        ),
...
async def run(client, model: str, question: str,
              tool_registry: dict, openai_tools: list[dict], verbose: bool = True,max_iterations: int = 5) -> dict:
    ...
    return {"answer": answer, "tool_calls": tool_call_log, "elapsed": elapsed, "iterations": iteration}
