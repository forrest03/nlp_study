...
def api_run_skill_step(skill_name: str, req: models.ExecuteRequest):
    ...
    if skill_name == "stock-dashboard":
        ...
        result = skill_executor._execute_tool("fetch_stock", arguments)
        if "error" in result:
            raise HTTPException(status_code=400, detail=result["error"])
        return result

    if skill_name == "flash-card":
        word = req.parameters.get("word", "")
        if not word:
            raise HTTPException(status_code=400, detail="缺少 word 参数")
        skill_executor._execute_tool("save_flashcard_data", req.parameters)
        result = skill_executor._execute_tool("generate_flashcard", {"word": word})
        return result
