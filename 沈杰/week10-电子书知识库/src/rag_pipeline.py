...
使用方式：
  ...
  python rag_pipeline.py --query "书中关于人工智能的观点"
  python rag_pipeline.py --query "..." --file book_name  # 指定书籍
...
SYSTEM_PROMPT = """你是一个专业的书籍问答助手，专门回答关于电子书内容的问题。

回答规则：
1. 只根据【参考资料】中的内容回答，不得引用或编造资料外的数据
2. 若参考资料不足以支撑回答，直接说"根据提供的资料无法回答此问题"
3. 引用具体内容时，在句末标注来源编号，如：这是一个重要观点[1]
4. 回答要准确、简洁，重点突出，避免无关废话"""
...
class VectorStore:
    ...
    def search(
        ...
        """
        向量检索，可选元数据过滤（按 source_file）。
        filter_meta 示例：{"source_file": "book.pdf"}
        """
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是检索查询优化专家。将用户的问题改写为更适合从电子书中检索信息的精确查询语句。"
                    "保留关键实体和概念，扩展相关关键词，不要超过50字。"
                    "直接输出改写后的查询语句，不要解释。"
                ),
            },
...
class RAGPipeline:
    ...
    def query(
        ...
        if top_score < SCORE_THRESHOLD and filter_meta is None:
            return {
                "answer": "根据书籍知识库未能找到与该问题相关的内容，建议直接查阅原始书籍。",
...
def main():
    parser = argparse.ArgumentParser(description="电子书 RAG 问答系统")
    parser.add_argument("--query",         type=str,  default=None)
    parser.add_argument("--file",          type=str,  default=None, help="来源文件名（不含扩展名），如 book")
    ...
    filter_meta = {}
    if args.file: filter_meta["source_file"] = args.file
