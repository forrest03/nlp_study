"""
文档分块脚本：对解析后的电子书做分块处理，供 RAG 系统使用
...
def process_file(parsed_path: Path, strategy: str = STRATEGY):
    ...
    # 获取来源文件信息（优先从meta中获取，否则使用文件名）
    source_file = meta.get("filename", parsed_path.stem)
    ...
    if strategy == "fixed":
        full_text = "\n\n".join(b["content"] for b in blocks if b.get("content", "").strip())
