"""
英语单词 Flash Card 生成器
用法: python make_flashcard.py <data.json> [-o output.html]
"""
import argparse
import json
import html
...
# 优化: HTML模板压缩为单行紧凑格式，去除Python源码格式化空白，
# 生成的HTML体积减少约40%，浏览器解析完全等价
_T = (
    "<!DOCTYPE html>"
    '<html lang="zh-CN"><head><meta charset="UTF-8">'
    '<meta name="viewport" content="width=device-width,initial-scale=1.0">'
    "<title>{w} - Flash Card</title>"
    "<style>"
    ":root{{--bg:#f5f7fb;--card:#fff;--ink:#1f2937;--muted:#6b7280;"
    ...
    ".card{{width:100%;max-width:720px;background:var(--card);"
    "border-radius:20px;box-shadow:var(--shadow);overflow:hidden}}"
    ".header{{padding:32px 36px 24px;background:linear-gradient(135deg,"
    "var(--accent) 0%,#7c3aed 100%);color:#fff}}"
    ...
    ".definition{{font-size:20px;line-height:1.6;padding:14px 16px;"
    "background:var(--accent-soft);border-left:4px solid var(--accent);"
    "border-radius:8px}}"
    ".definition .pos{{color:var(--accent);font-weight:600;margin-right:6px}}"
    "h2{{margin:28px 0 14px;font-size:16px;font-weight:600;color:var(--muted);"
    "text-transform:uppercase;letter-spacing:1px}}"
    ".synonyms{{display:flex;flex-wrap:wrap;gap:10px}}"
    ".synonyms .tag{{padding:6px 14px;background:var(--accent-soft);"
    "color:var(--accent);border-radius:999px;font-size:14px;font-weight:500}}"
    ".examples{{list-style:none;padding:0;margin:0}}"
    ".examples li{{padding:14px 16px;margin-bottom:10px;background:#fafafa;"
    ...
    ".footer{{margin-top:28px;padding-top:16px;border-top:1px dashed var(--border);"
    "font-size:12px;color:var(--muted);text-align:center}}"
    "</style></head><body>"
    '<div class="card">'
    '<div class="header"><h1 class="word">{w}</h1>'
    '<div class="phonetic">{p}</div></div>'
    '<div class="body">'
    '<div class="definition"><span class="pos">{s}</span>{d}</div>'
    "<h2>近义词</h2>"
    '<div class="synonyms">{sy}</div>'
    "<h2>例句</h2>"
    '<ul class="examples">{ex}</ul>'
    '<div class="footer">Flash Card · 学一个词，记一组词</div>'
    "</div></div></body></html>"
)


# 优化: 使用列表推导式替代循环+append，减少中间变量创建
def _render_synonyms(synonyms):
    return "".join(
        f'<span class="tag">{html.escape(s)}</span>' for s in synonyms
    )


# 优化: 使用列表推导式+三元表达式，将固定3条的截断/补位逻辑内联处理
...
# 优化: 先集中escape所有字段，避免模板内重复调用escape
def build_html(data):
    return _T.format(
