...
def main():
    base = _load_json(OUT / "baseline_probe.json")
    if base is None:
        raise FileNotFoundError("缺少 outputs/baseline_probe.json，请先跑 probe_baseline.py")

    post_full = _load_json(OUT / "post_train_probe.json")
    log_full = _load_json(OUT / "train_log.json")
    post_lora = _load_json(OUT / "post_train_probe_lora.json")
    log_lora = _load_json(OUT / "train_log_lora.json")

    reports = [("基线", base)]
    log_entries = []
    post_for_examples = None
    if post_full is not None:
        reports.append(("全量", post_full))
        post_for_examples = post_full
    if log_full is not None:
        log_entries.append(("full", log_full))
    if post_lora is not None:
        reports.append(("LoRA", post_lora))
        if post_for_examples is None:
            post_for_examples = post_lora
    if log_lora is not None:
        log_entries.append(("lora", log_lora))

    if len(reports) < 2:
        raise FileNotFoundError("缺少训练后 probe（post_train_probe.json 或 post_train_probe_lora.json）")

    n = next(iter(base.values()))["n"]
    print("=" * 96)
    print(f"训练前后对比（同一评估集，seed=42，{n} 题/难度；格式率 / greedy正确率 / pass@8）")
    print("=" * 96)
    print(fmt_table(reports))
    if post_for_examples is not None:
        print("\n" + "=" * 96)
        print("样例对照（greedy 解码）")
        print(fmt_examples(base, post_for_examples))
