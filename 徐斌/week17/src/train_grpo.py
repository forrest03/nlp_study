...
from probe_baseline import SYSTEM_PROMPT, make_problem, parse_output, pick_device, load_dtype, peak_memory_gb
...
def main():
    ...
    k_group = 4 if device == "mps" else 8
    print(f"训练设备：{device}  dtype={dtype}  num_generations={k_group}")

    config = GRPOConfig(
        output_dir=str(ckpt_dir),
        # 关键坑：本地 Qwen2-0.5B-Instruct 的 config.json 写着 torch_dtype=float16，
        # 不显式指定会被加载成 fp16 → AdamW 的 eps=1e-8 在 fp16 下溢出为 0 →
        # 0/0=NaN，一步训废整个模型（已实测定位）。CUDA 上必须显式 bfloat16。
        model_init_kwargs={
            "torch_dtype": dtype,
            **({"attn_implementation": "eager"} if device == "mps" else {}),
        },

        # ── GRPO 核心参数 ─────────────────────────────────────────────
        num_generations=k_group,
        beta=0.0,                   # KL 系数为 0：不加载参考模型，省 1GB 显存
        epsilon=0.2,                # PPO-clip 裁剪范围
        temperature=1.0,            # 采样温度：保持组内多样性
        max_prompt_length=128,
        max_completion_length=64,
        # 批次必须能被 num_generations 整除；MPS 上 K=4 降低 matmul 崩溃风险
        per_device_train_batch_size=k_group,
        gradient_accumulation_steps=4 if device != "mps" else 2,
