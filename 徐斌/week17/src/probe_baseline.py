...
MODEL_PATH = Path("~/Documents/www/py/llm/model/Qwen/Qwen2-0.5B-Instruct").expanduser()
OUT_PATH = ROOT / "outputs" / "baseline_probe.json"


def pick_device() -> str:
    """优先 CUDA，其次 Apple Silicon MPS，否则 CPU。"""
...
def load_dtype(device: str):
    # MPS 上 Qwen2 GQA + bfloat16/SDPA 会触发 mps_matmul incompatible dimensions
    if device in ("cpu", "mps"):
        return torch.float32
    return torch.bfloat16


def load_causal_lm(path, device):
    kwargs = {"torch_dtype": load_dtype(device)}
    if device == "mps":
        kwargs["attn_implementation"] = "eager"
        model = AutoModelForCausalLM.from_pretrained(path, **kwargs)
        return model.to(device)
    return AutoModelForCausalLM.from_pretrained(path, device_map=device, **kwargs)


def peak_memory_gb(device: str):
    if device == "cuda":
...
def _generate_once(model, tokenizer, batch, do_sample, nseq, max_new_tokens):
...
def generate(model, tokenizer, texts, do_sample, k=1, batch_size=16, max_new_tokens=64):
    ...
    on_mps = model.device.type == "mps"
    if on_mps:
        batch_size = 1  # 变长 padding 在 MPS 上会让 attention matmul 形状非法
    all_outputs = []
    for i in range(0, len(texts), batch_size):
        batch = texts[i : i + batch_size]
        nseq = k if do_sample else 1
        if on_mps and nseq > 1:
            grouped = [[] for _ in batch]
            for _ in range(nseq):
                decoded = _generate_once(model, tokenizer, batch, do_sample, 1, max_new_tokens)
                for j, text in enumerate(decoded):
                    grouped[j].append(text)
            all_outputs.extend(grouped)
            continue
        decoded = _generate_once(model, tokenizer, batch, do_sample, nseq, max_new_tokens)
        if do_sample:
