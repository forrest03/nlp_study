        ...
        if btype == "code":
            ...
            yield {
                ...
                "metadata": {
                    ...
                    "block_types": ["code"],
                    "code_lang":   block.get("code_lang") or "",
...
def build_chunk_id(doc_name: str, version: str, idx: int) -> str:
    slug = re.sub(r"[^\w\u4e00-\u9fff]+", "_", doc_name)[:30]
    ver  = version or "v0"
    return f"{slug}_{ver}_{idx:05d}"
...
def process_file(parsed_path: Path, strategy: str = STRATEGY):
    ...
    doc_name = meta.get("doc_name") or meta.get("stock_code") or "unknown"
    version  = meta.get("version") or meta.get("year") or ""
    ...
    for idx, chunk in enumerate(raw_chunks):
        chunk_id = build_chunk_id(doc_name, version, idx)
        chunk["chunk_id"]               = chunk_id
        chunk["metadata"]["doc_name"]   = doc_name
        chunk["metadata"]["version"]    = version
        chunk["metadata"]["doc_type"]   = meta.get("doc_type", "api")
...
def main():
    ...
    code_count  = sum(1 for c in all_chunks if "code" in c["metadata"].get("block_types", []))
    ocr_count   = sum(1 for c in all_chunks if c["metadata"].get("is_ocr"))
    logger.info(f"其中表格块: {table_count}  代码块: {code_count}  OCR块: {ocr_count}")
