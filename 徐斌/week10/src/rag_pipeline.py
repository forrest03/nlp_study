...
使用方式：
  ...
  python rag_pipeline.py --query "审核专家 API 的鉴权方式是什么？"
  python rag_pipeline.py --query "..." --section "接口说明"
...
SYSTEM_PROMPT = """你是一个专业的 API 文档问答助手，专门回答关于「审核专家 API 接入文档」的问题。

回答规则：
1. 只根据【参考资料】中的内容回答，不得引用或编造资料外的信息
2. 若参考资料不足以支撑回答，直接说"根据提供的文档无法回答此问题"
3. 涉及接口时，尽量给出：请求方法、路径、必填参数、响应字段、错误码等完整信息
4. 引用具体内容时，在句末标注来源编号，如：鉴权方式为 Bearer Token[1]
5. 回答简洁准确，优先使用文档原文表述，避免臆测"""
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是检索查询优化专家。将用户的问题改写为更适合从 API 接入文档中检索信息的精确查询语句。"
                    "保留关键实体（接口名、参数名、错误码、鉴权方式），扩展相关关键词，不要超过50字。"
                    "直接输出改写后的查询语句，不要解释。"
                ),
            },
...
def build_context(retrieved: list[dict]) -> tuple[str, list[dict]]:
    ...
    for i, item in enumerate(retrieved, 1):
        ...
        if version:
            label += f" {version}"
        code_lang = item.get("code_lang", "")
        if code_lang:
            label += f" · {code_lang} 代码"
        if section:
            label += f" · {section}"
        if page and page != -1:
            label += f" · 第{page}页"
...
class RAGPipeline:
    ...
    def query(
        ...
        if top_score < SCORE_THRESHOLD and filter_meta is None:
            return {
                "answer": "根据 API 文档知识库未能找到与该问题相关的内容，建议查阅原始接入文档。",
...
def main():
    parser = argparse.ArgumentParser(description="API 文档 RAG 问答")
    parser.add_argument("--query",         type=str,  default=None)
    parser.add_argument("--section",       type=str,  default=None, help="按章节关键词过滤，如 鉴权")
    ...
    filter_meta = {}
    if args.section:
        filter_meta["section"] = args.section
    if not filter_meta:
        filter_meta = None
