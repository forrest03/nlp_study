...
name: baoyu-diagram
description: >-
  创建专业的暗色主题 SVG 图表（架构图、流程图、时序图、结构图等）。
  当用户说"画个图""画一个架构图""diagram""flowchart""sequence diagram"时触发。
  输出始终是一个独立的 .svg 文件（本 harness 中可跳过 PNG 转换）。
version: 1.117.3
triggers:
  - 画个图
  - 架构图
...
7. **保存位置（本 harness）：** 用 `write_file` 保存到 `workspace/diagram/<topic-slug>.svg`

## 渐进式加载提示（Harness）

本 Skill 的完整定义较长。执行时请按需加载参考文件，不要一次读完所有 references：

1. 先 `activate_skill("baoyu-diagram")` 获取主流程与设计系统
2. 识别图表类型后，再用 `read_skill_file` 只读对应参考，例如：
   - 架构图 → `references/architecture.md`
   - 流程图 → `references/flowchart.md`
   - 时序图 → `references/sequence.md`
   - 结构图 → `references/structural.md`

## 流程

1. 从用户请求中识别图表类型
2. 若该类型有对应参考文件，用 `read_skill_file` 按需阅读（L2 披露）
3. 规划布局：列出所有组件，确定分组与流向，计算坐标
4. 按上述分层顺序编写 SVG
5. 检查间距规则——无重叠、图例在边界外、viewBox 足够大
6. 保存 SVG 到 `workspace/diagram/`
7. 向用户汇报文件路径（PNG 转换在本教学 harness 中可选，可跳过）
