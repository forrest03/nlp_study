...
def load_model(path, base_path=None):
    # attn_implementation="eager"：torch 2.6 MPS 的 SDPA/math 不支持 GQA
    # 广播（14 Q头 vs 2 KV头），eager 路径显式 repeat KV 头才不崩
    kwargs = dict(dtype=torch.bfloat16, device_map=DEVICE, attn_implementation="eager")
