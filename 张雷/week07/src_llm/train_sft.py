...
完整参数说明：
  --num_train -1      # 训练样本数，-1 用全部数据（默认）
  --epochs 3          # 训练轮数
  --batch_size 4      # 每步 batch 大小
  --grad_accum 4      # 梯度累积，等效 batch = 16
  --max_length 256    # NER JSON 输出比分类长，必须用 256
...
两种模式对比：
  ...
  显存需求     ~3GB                ~8~10GB
  checkpoint   outputs/sft_adapter/ outputs/sft_full_ckpt/
...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # peoples_daily 用 tokens 列表，join 为文本字符串
        user_text = "".join(item["tokens"])
