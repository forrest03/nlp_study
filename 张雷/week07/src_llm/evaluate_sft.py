...
evaluate_sft.py 自动识别 checkpoint 类型（目录含 adapter_config.json → LoRA，
否则 → 全量），并与 eval_llm.json 中的 API 结果自动对比。
"""
...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    """从 BIO 标注中提取 gold spans，格式 (text, type, start, end)。与 llm_ner.py 完全一致。"""
