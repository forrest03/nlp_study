...
与 cluener 版本的区别：
  1. 数据格式：已标注好的 BIO 标签（vs cluener 的 span 格式）
  2. 实体类型：3 类（PER/ORG/LOC）vs cluener 的 10 类
  3. 统计方式：直接统计 ner_tags（vs cluener 需要遍历 label 字典）
...
def collect_stats(records: list) -> dict:
    ...
    与 cluener 版本的区别：
      - cluener：遍历 label 字典的 span 格式 → 统计实体
      - peoples_daily：遍历 ner_tags 的 BIO 格式 → 统计实体
      需要从 BIO 序列中提取实体 span，然后统计。
    """
        ...
        for i, tag in enumerate(ner_tags):
            if tag.startswith("B-"):
                ...
                if current_entity_type is not None:
                    ...
                    entity_lengths.append(current_entity_len)
                    total_entities += 1
                    if current_entity_type not in entities_by_type:
                        entities_by_type[current_entity_type] = []
                    entities_by_type[current_entity_type].append("".join(current_entity_chars))

                # 开始新实体
                current_entity_type = tag[2:]
                current_entity_len = 1
                current_entity_chars = [tokens[i]] if i < len(tokens) else []

            elif tag.startswith("I-") and current_entity_type == tag[2:]:
                current_entity_len += 1
                if i < len(tokens):
                    current_entity_chars.append(tokens[i])

            else:
                # O 标签或类型不匹配的 I-，结束当前实体
                if current_entity_type is not None:
                    entity_type_counts[current_entity_type] += 1
                    entity_lengths.append(current_entity_len)
                    total_entities += 1
...
def plot_entity_distribution(stats_train: dict):
    ...
    bars = ax.bar(labels, values, color="#DD8452", alpha=0.85, edgecolor="white")
...
def plot_text_length_distribution(stats_train: dict):
    ...
    ax.hist(lengths, bins=50, color="#DD8452", alpha=0.8, edgecolor="white")
...
def main():
    ...
    print("\n探索完成！图表已保存到 outputs_pd/figures/")
