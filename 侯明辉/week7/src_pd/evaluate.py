...
与 cluener 版本的区别：
  1. 数据集：peoples_daily（3类实体：PER/ORG/LOC）
  2. 输出目录：outputs_pd/（与 cluener 的 outputs/ 隔离）
  3. 评估逻辑完全相同：seqeval entity-level F1 + 非法序列统计
...
def count_illegal_sequences(pred_seqs: list[list[str]]) -> dict:
    ...
    非法类型：
      ...
      - illegal_transition：O/B-X/I-X 后面跟 I-Y（X≠Y）或 O 后跟 I-X
    """
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"模型：{'BERT + CRF' if args.use_crf else 'BERT + Linear'}  |  数据集：人民日报 NER  |  评估集：{split_name}")
    print("=" * 70)
