...
教学重点：
  1. BIO 标注 → JSON span 格式的转换（与 cluener 的 span 标注不同）
  2. 3 类实体（PER/ORG/LOC）的 SFT 训练
  3. Loss masking：只在 JSON 输出部分计算 loss，prompt 全为 -100
  4. LoRA 高效微调 vs 全量微调（--full_ft 开关）

与 src_llm/train_sft.py 的区别：
  - 数据源：peoples_daily（BIO 格式）vs cluener（span 格式）
  - 实体类型：3 类 vs 10 类
  - 文本来源："".join(tokens) vs record["text"]
  - 目标 JSON：从 BIO 解析 vs 直接读取 label
...
def bio_to_spans(tokens: list[str], ner_tags: list[str]) -> list[dict]:
    """从 BIO 标注解析出实体 span 列表。
    ...
    输出：
      [{"text": "海淀区", "type": "LOC"}, {"text": "人民", "type": "PER"}]
    """
    entities = []
    i = 0
...
def record_to_target(record: dict) -> str:
    ...
    entities = bio_to_spans(tokens, ner_tags)
...
class SFTDataset(Dataset):
    """
    把人民日报 NER 数据（BIO 格式）转换为 chat-format SFT 训练样本。

    与 cluener 版本的关键区别：
      - 文本来源："".join(tokens) 而非 record["text"]
      - 目标生成：从 BIO 标签解析实体，而非直接读 label 字典

    Loss mask 结构：
      ┌──────────────────────────────────────────────────────────────┐
      │ <system>...<user>{text}<assistant>\n                         │  → -100
      │ {"entities": [{"text": "张三", "type": "PER"}]} <EOS>       │  → 真实 id
      └──────────────────────────────────────────────────────────────┘
    """
    ...
    def __getitem__(self, idx):
        ...
        # 人民日报数据需要拼接 tokens 得到原始文本
        text = "".join(item.get("tokens") or [])
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT NER 训练 — 人民日报（LoRA / 全量微调）")
...
def main():
    ...
    ckpt_dir   = output_dir / ("pd_sft_full_ckpt" if args.full_ft else "pd_sft_adapter")
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    device   = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    mode_str = "全量微调" if args.full_ft else "LoRA 微调"
    print(f"使用设备: {device}  |  微调模式: {mode_str}")
    print(f"数据集: 人民日报 NER（3 类实体：PER/ORG/LOC）")
    ...
    # ── 保存训练日志 ──────────────────────────────────────────────────────────
    log_tag  = "pd_full_ft" if args.full_ft else "pd_sft"
