"""
加载 SFT checkpoint（LoRA / 全量微调），在人民日报验证集上评估 NER entity-level F1

教学重点：
  1. 生成式 NER 的评估方式：JSON 解析 → span-level F1
  2. BIO 标注的 gold span 提取（与 cluener 的 span 格式不同）
  3. LoRA adapter 自动识别：目录含 adapter_config.json → LoRA，否则 → 全量

与 src_llm/evaluate_sft.py 的区别：
  - 数据源：peoples_daily（BIO 格式）vs cluener（span 格式）
  - 实体类型：3 类（PER/ORG/LOC）vs 10 类
  - 文本获取："".join(tokens) vs record["text"]
  - gold span 提取：从 BIO 解析 vs 从 label 字典读取
  - checkpoint 路径：pd_sft_adapter / pd_sft_full_ckpt

使用方式：
  python evaluate_sft.py                              # 评估 LoRA 模型（默认）
  python evaluate_sft.py --ckpt_dir ../outputs/pd_sft_full_ckpt  # 评估全量微调模型
...
ADAPTER_DIR = ROOT / "outputs" / "pd_sft_adapter"
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT NER 评估（人民日报）")
...
def main():
    ...
    if not ckpt_dir.exists():
        ...
        print("  LoRA（默认）保存到:   outputs/pd_sft_adapter/")
        print("  全量微调保存到:        outputs/pd_sft_full_ckpt/")
        return
    ...
    llm_zero_f1 = "?"
    llm_few_f1  = "?"
    llm_log_path = LOG_DIR / "eval_llm_peoples_daily.json"
    ...
    print(f"\n{'='*65}")
    print(f"LLM SFT NER 评估结果（人民日报 NER）")
    print(f"{'='*65}")
