...
- 当你收集足够信息，可以直接回答用户问题时，**必须以 FinalAnswer: 作为行首开始输出最终答案**
- 禁止不添加标签直接输出答案，不遵守格式会无法识别你的回答
- 如果还需要调用工具，严格使用 Thought / Action / ActionInput 格式
"""
...
def run(session_id: str, question: str, max_steps: int = 10) -> Generator[dict, None, None]:
...
def run_and_print(question: str, max_steps: int = 10):
    print(f"\n{'=' * 60}")
    print(f"问题: {question}")
    print(f"模型: {MODEL}  实现: 手写Prompt解析（命令行单次会话）")
    print('=' * 60)

    start = time.time()
    step_count = 0
    # 每次命令行提问生成独立临时session，不会复用历史，干净单次查询
    import uuid
    temp_session_id = "cli_temp_" + str(uuid.uuid4())

    # 调用带记忆生成器，但每次全新会话=等价单次无记忆
    for step_data in run(temp_session_id, question, max_steps=max_steps):
        step_count += 1
...
# sid = "local_test_001"
# # 第一轮
# for data in run(sid, "茅台2023毛利率", 10):
#     print(data)
# # 第二轮，自动带上一轮对话历史
# for data in run(sid, "那五粮液是多少", 10):
#     print(data)
