...
def run(question: str, max_steps: int = 10, memory: "ShortTermMemory | None" = None) -> Generator[dict, None, None]:
    """
    ...
    Args:
        question: 用户问题
        max_steps: 最大推理步数
        memory: 短期记忆实例，为 None 时自动创建（无状态模式）
    """
    ...
    for step in range(1, max_steps + 1):
        ...
        if parsed["type"] == "final":
            memory.add_message("assistant", f"Final Answer: {parsed['answer']}")
            yield {
                "step":    step,
                "type":    "final",
        ...
        # 将本步结果追加到对话历史（直接操作 memory 内部列表引用）
...
def run_and_print(question: str, max_steps: int = 10, memory: "ShortTermMemory | None" = None):
    print(f"\n{'='*60}")
    print(f"问题: {question}")
