...
if __name__ == "__main__":
    ...
    if args.mode == "manual":
        from react_manual import run_and_print, SYSTEM_PROMPT
        prompt = SYSTEM_PROMPT
    else:
        from react_function_calling import run_and_print, FC_SYSTEM_PROMPT
        prompt = FC_SYSTEM_PROMPT

    memory = ShortTermMemory(prompt)
    run_and_print(args.question, args.max_steps, memory=memory)
