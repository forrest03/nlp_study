...
教学重点：
  ...
  3. 与前两方式对比：CLI 是"工具实现层"，Function Call 是"意图生成层"，MCP 是"协议接入层"
     三者不互斥：run_cli/run_bash 本身也是用 Function Call 触发的，支持最多四轮工具调用
...
NAMED_COMMANDS = {
    ...
    "weather": {
        ...
        "arg_map": {"latitude": "--latitude", "longitude": "--longitude", "timezone": "--timezone"},
    },
}
...
NAMED_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "可查 A 股年报、地理位置（location）和按坐标查询的天气（weather）。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
                        ...
                        "description": "rag_search（查年报，需 query+可选 stock_code/year/top_k）/"
                                       ...
                                       " location（查坐标，需 city）/"
                                       " weather（查天气，需 latitude、longitude）",
                    },
                    "args": {
                        "type": "object",
                        "description": "命令参数。location: {city}; weather: {latitude, longitude, timezone?}。",
                    },
                },
                "required": ["command"],
            },
...
SYSTEM_PROMPT_NAMED = (
    ...
    "不在库内的公司请明确告知，不要臆测。查城市天气时，先调用 location 获取坐标和时区，"
    "再调用 weather；不得向 weather 传 city。本任务支持最多四轮工具调用。"
)
...
SYSTEM_PROMPT_BASH = (
    ...
    "查城市天气分两步：先执行 fincli location --city 南京，再将输出的坐标和时区传给 "
    "fincli weather --latitude 32.06 --longitude 118.80 --timezone Asia/Shanghai。"
