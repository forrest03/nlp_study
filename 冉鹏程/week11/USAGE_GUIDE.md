...
```
**内部流程**：
1. 手写 `TOOLS_SCHEMA`（年报工具 + 地理位置/天气两个独立工具的 JSON Schema）
...
2. 每个 Server `initialize()` 握手 → `list_tools()` 发现工具，打印已发现的工具；同时把 MCP `inputSchema` 转 OpenAI `parameters`（一次走完连接+发现+转 schema）
3. LLM 最多四轮闭环：`tool_call` → 查 `tool_registry` 路由到对应 `ClientSession` → `session.call_tool()` → 结果回填。城市天气会先定位再查天气。
4. 模型生成最终回答

**预期输出**：stderr 打印连接日志 + `共 4 个工具就绪`；stdout 打印工具调用与最终回答（含年报数据 + 宁德天气）。
...
# 第二步：将上一步返回的坐标和时区传给天气命令
fincli weather --latitude 26.66 --longitude 119.55 --timezone Asia/Shanghai
```

> 没装 fincli？将示例中的 `fincli` 替换为 `python mode_cli/cli/main.py` 即可。
...
`run_cli.py` 把上面的 `fincli` 包成 LLM 可调用的工具，支持最多四轮的工具链调用。有两种形态，用 `--mode` 切换：
...
location = get_location("宁德")
print(location)
if location["success"]:
    data = location["data"]
    print(get_weather(data["latitude"], data["longitude"], data["timezone"]))
```
