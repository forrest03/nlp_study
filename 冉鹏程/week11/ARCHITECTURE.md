...
> 项目聚焦"三种工具调用方式对比"。三种方式均支持最多四轮工具闭环，以处理“先定位、再查天气”的依赖调用；复杂 Agent 的长期规划仍属后续课程。
...
三种方式的 LLM 多轮闭环代码**几乎相同**（都是 `create(tools=) → tool_calls → 执行 → role=tool 回填 → 下一轮 create`，最多四轮），差异只在"工具定义从哪来"和"tool_call 怎么执行"——这是对比的教学核心。
...
4. **轮次边界**：四轮上限可覆盖“先定位、再查天气”等短链路；需要更多次检索、计划或反思时，应升级为带状态与终止条件的 Agent 循环。
...
│   └── weather_backend.py            # Open-Meteo：get_location + 按经纬度 get_weather
├── mode_function_call/
│   └── run_function_call.py          # 方式一：手写 schema + 最多四轮工具闭环
├── mode_mcp/
│   ├── servers/
│   │   ├── rag_server.py             # FastMCP Server（@mcp.tool 复用 rag_backend）
│   │   └── weather_server.py
│   └── run_mcp.py                    # 方式二：Host 连接 Server + 工具发现 + 最多四轮工具闭环
├── mode_cli/
│   ├── cli/
│   │   └── main.py                  # fincli 统一入口：search/list-companies/location/weather
