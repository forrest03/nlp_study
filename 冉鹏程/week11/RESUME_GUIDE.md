...
| 工具数 | 4 个（search_annual_report / list_companies / get_location / get_weather） | 三方式共用同一份后端 |
...
6. **为什么要限制工具调用轮次？**
   项目允许最多四轮：模型可先查 A，再依据 A 的结果查 B（例如城市定位后查天气）。上限可防止模型循环调用工具；需要更长链路时，应引入带状态、预算和终止条件的 Agent 循环。
