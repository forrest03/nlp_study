...
教学重点：
  ...
  2. 多轮闭环：模型输出 tool_call → 宿主执行工具 → 结果以 role=tool 回填 → 模型生成最终回答
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            "name": "get_location",
            "description": "将城市中文名解析为经纬度和时区。查询城市天气时必须先调用此工具。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "get_weather",
            "description": "仅按经纬度查询当前天气及未来3天预报；不得传入城市名。",
            "parameters": {
                "type": "object",
                "properties": {
                    "latitude": {"type": "number", "description": "纬度，范围 -90 至 90"},
                    "longitude": {"type": "number", "description": "经度，范围 -180 至 180"},
                    "timezone": {"type": "string", "description": "IANA 时区；来自 get_location，可选"},
                },
...
SYSTEM_PROMPT = (
    ...
    "涉及某城市天气时，必须先调用 get_location 获取 latitude、longitude、timezone，"
    "再使用这些值调用 get_weather；不得把 city 传给 get_weather。"
)
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    ...
    for _ in range(MAX_TOOL_ROUNDS):
        # 每轮都把上轮工具返回的数据交给模型，支持“定位 → 天气”的依赖调用。
        ...
        for tc in msg.tool_calls:
            ...
            if fn is None:
                ...
                try:
                    ...
                    result = f"工具执行失败：{e}"
            # OpenAI 的 tool 消息 content 必须是字符串。天气接口现返回结构化对象，
            # 在回填模型前序列化为 JSON；其它仍返回字符串的工具保持原样。
        ...
        # 达到工具轮次上限后禁止继续调用，要求模型基于已获取的数据直接作答。
        resp = client.chat.completions.create(
