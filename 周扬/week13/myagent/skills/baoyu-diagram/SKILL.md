...
处理这类任务时，优先使用这些工具：
- `read_text_file`：读取 `<skill_dir>/references/*.md` 布局参考
- `write_text_file`：写入生成好的 `.svg`
- `run_local_script`：执行 `<skill_dir>/scripts/main.ts`，把 `.svg` 转成 `@2x.png`
- `open_file`：打开生成好的 `.svg` 或 `.png` 预览

建议执行顺序：
1. 先根据用户意图识别图表类型
2. 用 `read_text_file` 读取对应 reference
3. 自己生成完整 SVG 文本
4. 用 `write_text_file` 保存为 `.svg`
5. 如需预览图像，再用 `run_local_script` 生成 `.png`
6. 最后用 `open_file` 打开结果文件
...
先确定本 SKILL.md 文件所在目录路径为 `<skill_dir>`。开始布局前，先读取对应图表类型的参考文件。参考文件位于 `<skill_dir>/references/`，包含详细的布局算法与示例。

### 架构图
→ 阅读 `<skill_dir>/references/architecture.md`

要点：数据流从左到右或从上到下。将相关服务用区域边界分组。层与层之间用总线/连接器。数据库放在底部或右侧。

### 流程图
→ 阅读 `<skill_dir>/references/flowchart.md`

要点：主流程从上到下。决策用菱形，出口箭头上标 Yes/No。起止用圆角矩形。主路径（happy path）用 Highlight 色。

### 时序图
→ 阅读 `<skill_dir>/references/sequence.md`

要点：角色为顶部方框，垂直虚线生命线，横向箭头表示消息（实线=同步，虚线=返回）。时间自上而下。激活条表示处理中。复杂时给消息编号。

### 结构图
→ 阅读 `<skill_dir>/references/structural.md`
...
确定本 SKILL.md 文件所在目录路径为 `<skill_dir>`。脚本路径：`<skill_dir>/scripts/main.ts`。
...
1. 从用户请求中识别图表类型
2. 若该类型有对应参考文件，先用 `read_text_file` 阅读
3. 规划布局：列出所有组件，确定分组与流向，计算坐标
4. 按上述分层顺序编写 SVG
5. 检查间距规则——无重叠、图例在边界外、viewBox 足够大
6. 用 `write_text_file` 保存 SVG 文件
7. 用 `run_local_script` 运行 `<skill_dir>/scripts/main.ts <svg-path>` 生成 @2x PNG
8. 用 `open_file` 打开 SVG 或 PNG，并将结果路径告知用户
