...
# 短期记忆
# messages 格式要求：必须包含 role 和 content 字段
messages = [
...
def run(question: str, max_steps: int = 10) -> Generator[dict, None, None]:
    ...
    for step in range(1, max_steps + 1):
        ...
        # 无论是工具调用还是直接回答，都记录到 messages 中
        # msg → ChatCompletionMessage 对象
        # 核心字段：
        # role	        str	            固定为 "assistant"
        # content	    str | None	    模型生成的文本内容（无工具调用时存在）
        # tool_calls	list | None    模型请求的工具调用列表（包含 id、function 等）
        # OpenAI 在构建 HTTP 请求时，自动调用 msg.model_dump()（或 .dict()）将其序列化为标准的 JSON 对象：
        # {
        #   "role": "assistant",
        #   "content": null,
        #   "tool_calls": [
        ...
        # 模型决定直接回答（无工具调用）
        # reason 可能的值包含："stop", "tool_calls"， "max_tokens", "content_filter"
