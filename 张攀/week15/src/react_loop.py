...
- Action Input 是该工具的参数字符串
- 每轮只调一次工具，等 Observation 再决定下一步
- 只要问题涉及天气查询（如「天气」「气温」「温度」「气温变化」等），必须用 get_weather_by_coords 获取天气信息，不要自己 web_search
"""
...
class ReActLoop:
    ...
    def run(self, question: str, on_step: Callable = None,
        ...
        for step_idx in range(self.max_steps):
            ...
            print(f"llm_out:--------------[{self.agent_name}]--------------------------\n{llm_out}\n")
            print("↓↓")
            ...
            print(
                f"Observation:--------------[{self.agent_name}]---------------------------\n"
                f"{action}({action_input}) ->{observation}\n")
