...
def get_site(city: str) -> tuple[float, float] | str:
    """
    查询指定城市的经纬度。
    """
    """
    ...
    with httpx.Client(timeout=10.0) as client:
        ...
        print(f"[{func_name}]：找到城市 {city_name}，位于{country} {admin1}，经纬度 {lat}, {lon}".strip())
        # print(f"找到城市：{city_name}，位于{country} {admin1}，经纬度 {lat}, {lon}".replace(" ", ""))

        return lon, lat
    
def get_site_weather(lon: float, lat: float) -> str:
    with httpx.Client(timeout=10.0) as client:
        try:
...
if __name__ == "__main__":
    ...
    lon, lat = get_site(args.city)
    if lon is not None and lat is not None:
        print(get_site_weather(lon, lat))
