...
from src.weather_backend import get_weather, get_site, get_site_weather  # noqa: E402
...
TOOLS_SCHEMA = [
    ...
    # },
    {
        ...
        "function": {
            ...
            "description": "查询指定经纬度坐标的当前天气及未来3天预报。",
            "parameters": {
                "type": "object",
                "properties": {
                    "lon": {"type": "float32", "description": "经度，如 119.5"},
                    "lat": {"type": "float32", "description": "纬度，如 26.66"},
                },
                "required": ["lon", "lat"],
            },
        },
...
TOOL_DISPATCH = {
    ...
    "get_site_weather": get_site_weather,
    "get_site": get_site,
}
...
SYSTEM_PROMPT = (
    "你是一名天气助手。回答用户关于天气的问题时，必须先调用 get_site 查询目标位置经纬度，"
    "**等待工具返回结果后**，再调用 get_site_weather 获取坐标位置天气信息，不要同时调用两个工具。"
    
    # 假设用户提问：“上海和北京的天气怎样？”
    # 没有该提示词：模型可能先输出 get_site(city="上海")，你执行后回填结果，模型再输出 get_site(city="北京")，
    # 你再执行回填，最后模型才能回答。这需要 2 次完整的 LLM 交互，耗时翻倍。
    # 有该提示词：模型会在一次响应中同时输出 get_site(city="上海") 和 get_site(city="北京") 两个 tool_call。
    # 你的代码循环执行这两个查询，将两个结果一起回填，模型仅需 1 次交互即可给出最终答案
    # "本回合你可以一次调用多个工具。"
)
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    ...
    #     created=1700000000,          # 时间戳
    #     model="deepseek-chat",       # 使用的模型名称
    #     choices=[                    # 候选列表（通常只有一个）
    #         Choice(
    #             index=0,             # 候选索引
    #             message=ChatCompletionMessage(  # ← 这里是 msg 的来源
    #                 role="assistant",
    #                 content="最终回答内容",
    #                 tool_calls=[     # 如果模型调用了工具，这里会有工具调用列表
    #                     {
    #                         "id": "call_xxx",
    #                         "type": "function",
    #                         "function": {
    ...
    #                 ]
    #             ),
    #             finish_reason="stop"  # 结束原因：stop/tool_calls/length等
    #         )
    #     ],
    #     usage={                      # Token 使用统计
    #         "prompt_tokens": 100,
    ...
    while True:
        ...
        for tc in msg.tool_calls:
            ...
            if fn is None:
                ...
                try:
                    ...
                    if not isinstance(result, str):
                        if name == "get_site" and isinstance(result, tuple):
                            result = {"lon": result[0], "lat": result[1]}
            ...
            if verbose:
                print(f"  → [result] {preview}{'...' if len(result or '') > 120 else ''}\n")
...
def main():
    ...
    parser.add_argument("--question", "-q", default="上海、北京和深圳的天气怎样？", help="单个问题")
