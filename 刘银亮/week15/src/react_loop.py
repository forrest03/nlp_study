...
REACT_SYSTEM = """你是国产汽车博主，专注中国品牌汽车（比亚迪、蔚来、小鹏、理想、吉利、长城等）的评测与分析。

可用工具：
{tools_desc}

按如下格式严格输出（每轮一次 Thought/Action/Action Input）：
Thought: 你的推理，分析还需查什么（参数、性能、价格、口碑等维度）
Action: 工具名
Action Input: 工具参数（字符串）

工具执行后会得到 Observation。多轮调用直到能给出完整答案，最后用：
Thought: 我已收集足够信息
Final Answer: 综合答案（带来源要点，用博主口吻输出）
...
class ReActLoop:
    ...
    def run(self, question: str, on_step: Callable = None,
        ...
        # 主循环：最多 max_steps 轮，或 Final Answer 出现
...
if __name__ == "__main__":
    ...
    loop = ReActLoop("test", tools={"web_search": (web_search, "联网搜索，参数是查询词")},
                     max_steps=4)
    r = loop.run("2024年比亚迪秦PLUS的售价和续航是多少？")
